 ...
 */
require_once APD_LIB_DIR . 'AsaZend/Cache/Backend.php';

/**
 * @see AsaZend_Cache_Backend_Interface
 */
require_once APD_LIB_DIR . 'AsaZend/Cache/Backend/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class AsaZend_Cache_Backend_ZendPlatform extends AsaZend_Cache_Backend implements AsaZend_Cache_Backend_Interface
{
    /**
     * internal ZP prefix
     */
    ...
    public function __construct(array $options = array())
    {
        if (!function_exists('accelerator_license_info')) {
            AsaZend_Cache::throwException('The Zend Platform extension must be loaded for using this backend !');
        }
        if (!function_exists('accelerator_get_configuration')) {
            $licenseInfo = accelerator_license_info();
            AsaZend_Cache::throwException('The Zend Platform extension is not loaded correctly: '.$licenseInfo['failure_reason']);
        }
        $accConf = accelerator_get_configuration();
        if (@!$accConf['output_cache_licensed']) {
            AsaZend_Cache::throwException('The Zend Platform extension does not have the proper license to use content caching features');
        }
        if (@!$accConf['output_cache_enabled']) {
            AsaZend_Cache::throwException('The Zend Platform content caching feature must be enabled for using this backend, set the \'zend_accelerator.output_cache_enabled\' directive to On !');
        }
        if (!is_writable($accConf['output_cache_dir'])) {
            AsaZend_Cache::throwException('The cache copies directory \''. ini_get('zend_accelerator.output_cache_dir') .'\' must be writable !');
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        if (!($specificLifetime === false)) {
            $this->_log("AsaZend_Cache_Backend_ZendPlatform::save() : non false specifc lifetime is unsuported for this backend");
        }
     ...
     * AsaZend_Cache::CLEANING_MODE_NOT_MATCHING_TAG => unsupported
    ...
    public function clean($mode = AsaZend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
            ...
            case AsaZend_Cache::CLEANING_MODE_NOT_MATCHING_TAG:
                $this->_log("AsaZend_Cache_Backend_ZendPlatform::clean() : CLEANING_MODE_NOT_MATCHING_TAG is not supported by the Zend Platform backend");
                return false;
                break;
     ...
     * @param  string $mode The same parameter as in AsaZend_Cache_Backend_ZendPlatform::clean()
    ...
    private function _clean($dir, $mode)
    {
        ...
        while (false !== ($file = $d->read())) {
            ...
            if (is_dir($file)) {
                ...
                if ($mode == AsaZend_Cache::CLEANING_MODE_ALL) {
                    $result = ($this->_remove($file)) && ($result);
                } else if ($mode == AsaZend_Cache::CLEANING_MODE_OLD) {
    ...
    private function _remove($file)
    {
        if (!@unlink($file)) {
            # If we can't remove the file (because of locks or any problem), we will touch
            # the file to invalidate it
            $this->_log("AsaZend_Cache_Backend_ZendPlatform::_remove() : we can't remove $file => we are going to try to invalidate it");
            if ($this->_directives['lifetime'] === null) {
                return false;
            }
            if (!file_exists($file)) {
