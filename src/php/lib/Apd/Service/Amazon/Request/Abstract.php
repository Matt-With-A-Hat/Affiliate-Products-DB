...
require_once APD_LIB_DIR . 'Apd/Service/Amazon/Request/Exception.php';

abstract class Apd_Service_Amazon_Request_Abstract
{
    /**
     ...
     * Constructor
     *
*@param Apd_Service_Amazon $asa
     */
    public function __construct( Apd_Service_Amazon $asa)
    {
        $this->_apd = $asa;
    }
    ...
    public function send(array $url_params)
    {
        ...
        if (!$request_url = $this->getRequestUrl()) {
            throw new Apd_Service_Amazon_Request_Exception('Error in building request url');
        }
        ...
        if ($this->_response->isError()) {
            ...
            throw new Apd_Service_Amazon_Request_Exception($error);
        }
...
*@param array $url_params
     *
*@throws Apd_Service_Exception
     */    
    public function build(array $url_params)
    {
        if (empty($url_params) && !is_array($url_params)) {
            throw new Apd_Service_Exception('Invalid URL params');
        }

        if ($this->_apd->getLocale() == null) {
            throw new Apd_Service_Exception('Missing locale');
        }        
        ...
        $base_url = $this->_apd->getEndpoint() .
            '?Service=AWSECommerceService&AWSAccessKeyId=%s&Version=%s&AssociateTag=%s&';
            
        $base_url = sprintf($base_url, $this->_apd->getAccessKeyId(),
	        Apd_Service_Amazon::$api_version, $this->_apd->getAssociateTag());
        ...
        $signstr = base64_encode(
            hash_hmac('sha256', $signstr, $this->_apd->getAccessKeySecret(), true));
