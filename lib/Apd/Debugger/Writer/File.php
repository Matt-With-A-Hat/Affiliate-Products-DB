...
require_once APD_LIB_DIR . 'Apd/Debugger/Writer/Abstract.php';

class Apd_Debugger_Writer_File extends Apd_Debugger_Writer_Abstract
{
    /**
     * @var string
     */
    ...
    protected function _initFile()
    {
        if (empty($this->_filename) || !is_writable($this->_filename)) {
            require_once APD_LIB_DIR . 'Apd/Debugger/Exception.php';
            throw new Apd_Debugger_Exception('Debug file could not be initialized');
        }
        
        if (!$contents = @file_get_contents($this->_filename)) {

            $contents = '<?php' . PHP_EOL;
            $contents .= 'if (!defined(APD_DEBUG_READ)) exit("invalid access");' . PHP_EOL;
            $contents .= '?>' . PHP_EOL;
    ...
    public function setFilename ($filename=null)
    {
        ...
        if (!file_exists($filename) && file_put_contents($filename, '') === false) {
            require_once APD_LIB_DIR . 'Apd/Debugger/Exception.php';
            throw new Apd_Debugger_Exception('Debug filename could not be set');
        }
