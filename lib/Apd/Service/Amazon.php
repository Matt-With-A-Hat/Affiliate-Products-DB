<?php
/**
 * Affiliate Product DB - Wordpress Plugin
 *
 */

require_once APD_LIB_DIR . 'Apd/Service/Exception.php';
require_once APD_LIB_DIR . 'Apd/Service/Amazon/Exception.php';
require_once APD_LIB_DIR . 'Apd/Service/Amazon/Interface.php';
require_once APD_LIB_DIR . 'Apd/Service/Amazon/Request.php';

class Apd_Service_Amazon implements Apd_Service_Amazon_Interface
{
    /**
     ...
     * @var string
     */
    protected $_connection_type = 'http';
     ...
     * @var Apd_Service_Amazon_Request_Abstract
     */
    protected $_request;
...
*@param string $access_key
     ...
     *
*@throws Apd_Service_Amazon_Exception
     */
    public function __construct($access_key, $secret, $tag, $locale, $connection_type = null)
    {
        if (empty($access_key)) {
            throw new Apd_Service_Amazon_Exception('Missing access key');
        }
        if (empty($secret)) {
            throw new Apd_Service_Amazon_Exception('Missing secret access key');
        }
        if (empty($tag)) {
            throw new Apd_Service_Amazon_Exception('Missing associate tag');
        }
        if (empty($locale)) {
            throw new Apd_Service_Amazon_Exception('Missing locale');
        }
        ...
        $this->_request = Apd_Service_Amazon_Request::factory($this);
    }
     ...
     * @param null $connection_type
     *
*@return Apd_Service_Amazon
     */
    public static function factory($access_key, $secret, $tag, $locale, $connection_type = null)
    {
        $Apd = new Apd_Service_Amazon($access_key, $secret, $tag, $locale, $connection_type);

         if (getenv('APD_APPLICATION_ENV') == 'development' || get_option('_apd_debug') == true) {
            require_once APD_LIB_DIR . 'Apd/Service/Amazon/Debug.php';
            $ApdDebug = new Apd_Service_Amazon_Debug($Apd);
            return $ApdDebug;
         }
    ...
    {
        if (empty($asin)) {
            throw new Apd_Service_Amazon_Exception('Missing ASIN');
        }
        ...
        if ($errors->length > 0) {
            ...
            require_once APD_LIB_DIR . 'Apd/Service/Amazon/Error.php';
            return new Apd_Service_Amazon_Error($errors, $asin);
    ...
    public function itemSearch(array $options)
    {
        if (empty($options) || !is_array($options)) {
            throw new Apd_Service_Amazon_Exception('Invalid ItemSearch options');
        }
    ...
    public function setLocale($locale)
    {
        if (!in_array($locale, array_keys(self::$_endpoints))) {
            throw new Apd_Service_Amazon_Exception( 'Invalid locale: ' . $locale);
        }
        $this->_locale = $locale;
    }
