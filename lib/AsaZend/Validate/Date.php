 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class AsaZend_Validate_Date extends AsaZend_Validate_Abstract
{
    ...
    protected $_messageTemplates = array(
        self::INVALID        => "Invalid type given. String, integer, array or AsaZend_Date expected",
     ...
     *
     * @param  string|AsaZend_Config $options OPTIONAL
     * @return void
     */
    public function __construct($options = array())
    {
        ...
        if (!array_key_exists('locale', $options)) {
            ...
            if (AsaZend_Registry::isRegistered('AsaZend_Locale')) {
                $options['locale'] = AsaZend_Registry::get('AsaZend_Locale');
            }
        }
     ...
     * according to AsaZend_Date, see AsaZend_Date::isDate()
     *
     * @param  string|array|AsaZend_Date $value
     * @return boolean
     */
    public function isValid($value)
    {
        if (!is_string($value) && !is_int($value) && !is_float($value) &&
            !is_array($value) && !($value instanceof AsaZend_Date)) {
        ...
        if (($this->_format !== null) || ($this->_locale !== null) || is_array($value) ||
             $value instanceof AsaZend_Date) {
            require_once APD_LIB_DIR . 'AsaZend/Date.php';
            if (!AsaZend_Date::isDate($value, $this->_format, $this->_locale)) {
    ...
    private function _checkFormat($value)
    {
        try {
            ...
            $parsed = AsaZend_Locale_Format::getDate($value, array(
            ...
            if (isset($parsed['year']) and ((strpos(strtoupper($this->_format), 'YY') !== false) and
                ...
                $parsed['year'] = AsaZend_Date::getFullYear($parsed['year']);
            }
        } catch (Exception $e) {
