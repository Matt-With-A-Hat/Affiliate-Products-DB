...
/**
 * @see AsaZend_Http_Client_Adapter_Stream
 */
require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Stream.php';

/**
 * An adapter class for AsaZend_Http_Client based on the curl extension.
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class AsaZend_Http_Client_Adapter_Curl implements AsaZend_Http_Client_Adapter_Interface, AsaZend_Http_Client_Adapter_Stream
{
    /**
     * Parameters array
     *
     ...
     *
     * @return void
     * @throws AsaZend_Http_Client_Adapter_Exception
     */
    public function __construct()
    {
        if (!extension_loaded('curl')) {
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
            throw new AsaZend_Http_Client_Adapter_Exception('cURL extension has to be loaded to use this AsaZend_Http_Client adapter.');
        }
     ...
     * @return AsaZend_Http_Client_Adapter_Curl
     */
    public function setConfig($config = array())
    {
        if ($config instanceof AsaZend_Config) {
            ...
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
            throw new AsaZend_Http_Client_Adapter_Exception(
                'Array or AsaZend_Config object expected, got ' . gettype($config)
            );
        }
     ...
     * @param  mixed $value
     * @return AsaZend_Http_Adapter_Curl
     */
     ...
     * @return void
     * @throws AsaZend_Http_Client_Adapter_Exception if unable to connect
     */
    public function connect($host, $port = 80, $secure = false)
    {
        ...
        if (!$this->_curl) {
            ...
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
            throw new AsaZend_Http_Client_Adapter_Exception('Unable to Connect to ' .  $host . ':' . $port);
        }

        if ($secure !== false) {
            // Behave the same like AsaZend_Http_Adapter_Socket on SSL options.
     ...
     *
     * @param  string        $method
     * @param  AsaZend_Uri_Http $uri
     * @param  float         $http_ver
     * @param  array         $headers
     * @param  string        $body
     * @return string        $request
     * @throws AsaZend_Http_Client_Adapter_Exception If connection fails, connected to wrong host, no PUT file defined, unsupported method, or unsupported cURL option
     */
    public function write($method, $uri, $httpVersion = 1.1, $headers = array(), $body = '')
    {
        ...
        if (!$this->_curl) {
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
            throw new AsaZend_Http_Client_Adapter_Exception("Trying to write but we are not connected");
        }

        if ($this->_connected_to[0] != $uri->getHost() || $this->_connected_to[1] != $uri->getPort()) {
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
            throw new AsaZend_Http_Client_Adapter_Exception("Trying to write but we are connected to the wrong host");
        }
        ...
        $curlValue = true;
        switch ($method) {
            case AsaZend_Http_Client::GET:
                $curlMethod = CURLOPT_HTTPGET;
                break;

            case AsaZend_Http_Client::POST:
                $curlMethod = CURLOPT_POST;
                break;

            case AsaZend_Http_Client::PUT:
                ...
                if (isset($this->_config['curloptions'][CURLOPT_INFILE])) {
                    ...
                    if (!isset($this->_config['curloptions'][CURLOPT_INFILESIZE])) {
                        require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
                        throw new AsaZend_Http_Client_Adapter_Exception("Cannot set a file-handle for cURL option CURLOPT_INFILE without also setting its size in CURLOPT_INFILESIZE.");
                    }
            ...
            case AsaZend_Http_Client::DELETE:
                $curlMethod = CURLOPT_CUSTOMREQUEST;
                $curlValue = "DELETE";
                break;

            case AsaZend_Http_Client::OPTIONS:
                $curlMethod = CURLOPT_CUSTOMREQUEST;
                $curlValue = "OPTIONS";
                break;

            case AsaZend_Http_Client::TRACE:
                $curlMethod = CURLOPT_CUSTOMREQUEST;
                $curlValue = "TRACE";
                break;
            
            case AsaZend_Http_Client::HEAD:
                $curlMethod = CURLOPT_CUSTOMREQUEST;
                $curlValue = "HEAD";
                break;
            ...
            default:
                ...
                require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
                throw new AsaZend_Http_Client_Adapter_Exception("Method currently not supported");
        }

        if(is_resource($body) && $curlMethod != CURLOPT_PUT) {
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Adapter/Exception.php';
            throw new AsaZend_Http_Client_Adapter_Exception("Streaming requests are allowed only with PUT");
        }
         ...
         */
        if ($method == AsaZend_Http_Client::POST) {
        ...
        } elseif ($method == AsaZend_Http_Client::PUT) {
            ...
            foreach ((array)$this->_config['curloptions'] as $k => $v) {
                if (!in_array($k, $this->_invalidOverwritableCurlOptions)) {
                    if (curl_setopt($this->_curl, $k, $v) == false) {
                        ...
                        throw new AsaZend_Http_Client_Exception(sprintf("Unknown or erroreous cURL option '%s' set", $k));
                    }
                }
            }
        }
        ...
        if (empty($this->_response)) {
            require_once APD_LIB_DIR . 'AsaZend/Http/Client/Exception.php';
            throw new AsaZend_Http_Client_Exception("Error in cURL request: " . curl_error($this->_curl));
        }

        // cURL automatically decodes chunked-messages, this means we have to disallow the AsaZend_Http_Response to do it again
     ...
     * @return AsaZend_Http_Client_Adapter_Socket
     */
